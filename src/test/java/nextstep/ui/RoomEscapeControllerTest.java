...
class RoomEscapeControllerTest extends ApiDocument {
  ...
  void 예약_생성된다() throws Exception {
    //given
    var reservationReq = RoomReservationReq.builder()
        .date(LocalDate.now())
        .time("13:00")
        .name("gump")
        .build();
  ...
  private ResultActions 예약_생성_요청(RoomReservationReq reservationReq) throws Exception {
  ...
  private void 예약_생성_성공함(ResultActions actual) throws Exception {
    actual.andExpect(status().isCreated())
        .andDo(print())
        .andDo(toDocument("room-escape-create"));
  }
}
